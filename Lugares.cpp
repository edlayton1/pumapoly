...
bool read_bin( std::vector <Vertex>& lista )
{
    ...
    if( file_in == NULL )
    {
        std::cout << "El archivo no pudo ser abierto para lectura.\n";
        return false; 
    }
    else
    {
        ...
        {
            Vertex a;
            fread(&a, sizeof(Vertex), 1, file_in);

            if( !feof(file_in)  )
            {
                std::cout << "Lugar: " << a.get_name() << "Descripción" << a.get_descripcion() << "\n";
                lista.push_back( a );
            }
        }
...
Graph* crearTablero()
{
    ...
    /*std::vector<Vertex> Lugares;

    read_bin(Lugares);

     for(auto &a : Lugares){
        Tablero->add_vertex(a);
        std::cout << a.get_name() << std::endl;
        std::cout << a.get_descripcion()<< std::endl;
